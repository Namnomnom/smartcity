...
class Model(nn.Module):
    ...
    def load_vgg(self):
        ...
        new_dict = {}
        for i in range(16):
            new_dict['conv' + new_name[i] + '.conv.weight'] = state_dict['features.' + str(old_name[i]) + '.weight']
            new_dict['conv' + new_name[i] + '.conv.bias'] = state_dict['features.' + str(old_name[i]) + '.bias']
...
class BackEnd(nn.Module):
    ...
    def forward(self, *input):
        ...
        feature = self.reg_layer(F.interpolate(feature, scale_factor=2, mode='bilinear', align_corners=True))
